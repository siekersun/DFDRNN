...
class BaseModel(pl.LightningModule):
    ...
    def contract_loss(self,embedding1, embedding2, temperature):
        normalized1 = F.normalize(embedding1, p=2, dim=-1)
        normalized2 = F.normalize(embedding2, p=2, dim=-1)
        sim = torch.exp(torch.matmul(normalized1, normalized2.T) / temperature)
        positive = sim.diag()
        negative = torch.sum(torch.cat([sim, sim.T], 1), dim=1) - 2 * positive
        loss = -torch.sum(torch.log(positive * (1.0 / (negative))))
        return loss
