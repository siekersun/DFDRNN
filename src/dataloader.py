...
from .utils import select_topk,random_keep_k
...
class DRDataset():
    def __init__(self, dataset_name="Gdataset", drug_neighbor_num=15, disease_neighbor_num=15,feature_topk=10,seed=42):
        ...
        self.drug_neighbor_num = drug_neighbor_num
        
        if dataset_name=="lagcn":
            self.drug_edge = self.build_graph(self.drug_sim + np.eye(self.drug_num), drug_neighbor_num)
            self.disease_edge = self.build_graph(self.disease_sim + np.eye(self.disease_num), disease_neighbor_num)
        else:
...
class Dataset():
    def __init__(self, dataset, mask, fill_unkown=True, stage="train", **kwargs):
        ...
        self.drug_sim = select_topk(torch.from_numpy(dataset.drug_sim).float(),dataset.feature_topk)
        self.disease_sim = select_topk(torch.from_numpy(dataset.disease_sim).float(),dataset.feature_topk)
        # self.drug_sim = torch.from_numpy(dataset.drug_sim).float()
        # self.disease_sim = torch.from_numpy(dataset.disease_sim).float()
    ...
    def getx_s(self):
        matrix_zeros = torch.zeros_like(self.mask).float()

        u_embedding = torch.cat([self.drug_sim, matrix_zeros], 1)
        v_embedding = torch.cat([matrix_zeros.T, self.disease_sim], 1)
        embedding = torch.cat([u_embedding, v_embedding], 0)
        return embedding
...
def load_DRIMC(root_dir="dataset/drimc", name="c", reduce=True):
    """ C drug:658, disease:409 association:2520 (False 2353)
        PREDICT drug:593, disease:313 association:1933 (Gdataset)
...
def load_LAGCN(root_dir="dataset/lagcn"):
    ...
    # dd += np.eye(dd.shape[0])
    # rr += np.eye(rr.shape[0])
