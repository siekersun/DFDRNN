...
def train(config, model_cls=DFDRNN):
    time_stamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    datasets = DRDataset(dataset_name=config.dataset_name, drug_neighbor_num=config.drug_neighbor_num,
                         disease_neighbor_num=config.disease_neighbor_num,feature_topk=config.feature_topk,seed = config.seed)
    log_dir = os.path.join(f"{config.comment}", f"{config.split_mode}-{config.n_splits}-fold", f"{config.dataset_name}", 
                           f"{config.seed}",f"{config.drug_neighbor_num}",f"{model_cls.__name__}", f"{time_stamp}")
...
def parse(print_help=False):
    ...
    parser.add_argument("--model", default="DFDRNN", type=str)
    parser.add_argument("--epochs", default=800, type=int)
    parser.add_argument("--debug", default=False, action="store_true")
    parser.add_argument("--profiler", default=False, type=str)
    parser.add_argument("--comment", default="output", type=str, help="experiment name")
    parser = DRDataset.add_argparse_args(parser)
    parser = CVDataset.add_argparse_args(parser)
    parser = DFDRNN.add_model_specific_args(parser)
